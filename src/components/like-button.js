...
export default function LikeButton({ slug }) {
  ...
  const [incrementLikes, { data: incrementedData }] = useMutation(
    INCREMENT_LIKES
  )

  const likes =
    incrementedData?.update_analytics?.returning[0]?.likes ||
    data?.analytics[0]?.likes
  ...
  return (
    <Fragment>
      ...
      {liked && <CollapsedPow className="h-10 w-10 md:h-16 md:w-16" />}
